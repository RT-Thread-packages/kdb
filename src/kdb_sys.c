...
void kdb_printf_output(const char *fmt, ...)
{
    va_list args;
    rt_size_t length;
    char log_buf[KDB_PRINT_BUFF_SIZE];
...
struct kdb sys_kdb;
rt_uint8_t sys_kdb_buff[KDB_SYS_BUFF_SIZE];
#endif
...
int kdb_sys_init(void)
{
    ...
    kdb_init(&sys_kdb, sys_kdb_buff, KDB_SYS_BUFF_SIZE);
    return 0;
}
